...
---@return table 'A table of callbacks which handle input events.'
--- @see NobleScene.inputHandler
...
---@param __inputHandler? table 'A table of callbacks which handle input events. Default: nil'
--- @see NobleScene.inputHandler
--- @see clearHandler
--- @see setEnabled
...
---@param __value boolean 'Set to false to halt input. Set to true to resume accepting input.'
--- @see getHandler
--- @see clearHandler
...
---@return boolean 'Returns true if the input system is enabled. Returns false if `setEnabled(false)` was used, or if currentHandler is `nil`.'
...
---@param __active boolean 'Set true to start showing the on-screen crank indicator. Set false to stop showing it.'
---@param __evenWhenUndocked? boolean 'Set true to show the crank indicator even if the crank is already undocked (`__active` must also be true). Default: false'
...
--
---@return boolean 'Is the crank indicator active?'
---@return boolean 'Is the crank indicator being forced when active, even when the crank is undocked?'
...
---@param __getStoredValues? boolean 'If true, this method will simply return the most recently stored values, rather than use the accelerometer to check for new ones. Default: false'
---@return str 'The named orientation of the device (a pseudo enum Noble.Input.ORIENTATION_XX)'
---@return list 'Accelerometer values, where list[1] is x, list[2] is y and list[3] is z'
